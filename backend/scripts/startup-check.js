...
// Run the centralized envCheck script which provides a clear, CI-friendly exit code
try {
  const envCheck = require('./envCheck');
  envCheck.run();
} catch (err) {
  // If envCheck fails to load, log and continue with best-effort checks below
  console.warn('Env check module failed to load or run:', err && err.message);
}
...
// If using in-memory DB for local tests, it's ok for MONGODB_URI to be missing
const useInMemory = (v) => v && ['1', 'true', 'True', 'TRUE'].includes(v);

if (useInMemory(process.env.USE_IN_MEMORY_DB)) {
  console.log(`${colors.yellow}ℹ️  USE_IN_MEMORY_DB is enabled — skipping MONGODB_URI check and remote DB connectivity test${colors.reset}`);
} else {
  // If not using in-memory DB, ensure MONGODB_URI is present
  if (!process.env.MONGODB_URI) {
    console.log(`${colors.red}❌ Missing required: MONGODB_URI${colors.reset}`);
    hasErrors = true;
  }
}
...
async function runAsyncChecks() {
  if (process.env.MONGODB_URI) {
    ...
    console.log(`${colors.green}✅ In-memory DB enabled — skipping remote MongoDB connectivity test${colors.reset}`);
  }
